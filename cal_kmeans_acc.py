...
#     set_labels = [(int(labels[i]) - offset) for i in range(len(labels))]
#     set_labels = [(set_labels[i] + n_classes) if set_labels[i] < 0 else set_labels[i] for i in range(len(set_labels))]

#     correct_or_fail = []
#     for i in range(len(kmeans_results)) :
#         correct_or_fail.append((int(kmeans_results[i]) == set_labels[i]))
        
#     acc = correct_or_fail.count(True) / len(correct_or_fail)
    
#     print("label offset : " + str(offset))
#     print(f"acc : {acc}")



    
def split_cluster_acc_v2(y_true, y_pred):
    """
